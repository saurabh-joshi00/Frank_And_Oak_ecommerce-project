...
export default function ViewSize() {
    ...
    if(checkBoxValue.length != sizes.length){
        const data = [];
        sizes.forEach((v ) => {
        data.push(v._id);
        })
    ...
    const deleteAll = () => {
        if(confirm('Are you sure you want to delete ?')){
          axios.post('http://localhost:1500/api/admin/sizes/delete',{
            id : checkBoxValue
          })
          .then( (success) => {
    ...
    useEffect( () => {
        axios.post('http://localhost:1500/api/admin/sizes',{
            page : currentPage,
            limit : 2
        })
        .then( (success) => {
          if(success.data.status == true){
            setSizes(success.data.data);
          } else {
            setSizes([]);
          }
                        ...
                        <thead className="text-sm text-gray-700 uppercase bg-gray-50 ">
                            <tr>
                                <th scope="col" className="px-6 py-3" width="100px">
                                    ...
                                    checked = { (checkBoxValue.length == sizes.length ) ? 'true' : ''  }
                                    />
                                    Select All
                                </th>
                                ...
                                sizes.map( (v, i) => {
                                    return(
                                        <tr className="bg-white border-b ">
                                            ...
                                            onClick={ () => singleSelectHandler(v._id) }
                                            ...
                                            | 
                                            <Link to={`/size/update-size/${ v._id }`}>
