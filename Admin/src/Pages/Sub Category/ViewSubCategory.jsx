...
export default function ViewSubCategory() {
    ...
    const selectAllHandler = () => {
        ...
        if(checkBoxValue.length != subCategories.length){
        const data = [];
    ...
    const deleteAll = () => {
        ...
        axios.post('http://localhost:1500/api/admin/sub-categories/delete',{
            id : checkBoxValue
        })
        .then( (success) => {
    ...
    const changeStatus = () => {
        axios.post('http://localhost:1500/api/admin/sub-categories/change-status',{
        id : checkBoxValue,
        })
        .then( (success) => {
        if(success.data.status === true){
    ...
    useEffect( () => {
        axios.post('http://localhost:1500/api/admin/sub-categories',{
          page : currentPage,
          limit : 2,
        })
        .then( (success) => {
                        ...
                        <tr>
                            <th scope="col" className="px-6 py-3" width="100px">
                                <input name='deleteCheck' id="purple-checkbox" type="checkbox" value="" className="w-4 h-4 text-purple-600 bg-gray-100 border-gray-300 rounded focus:ring-purple-500 " onChange={ selectAllHandler }
                                  checked = { (checkBoxValue.length == subCategories.length ) ? 'true' : ''  }
                                />
                                Select All
                            </th>
                            <th scope="col" className="px-6 py-3" width="100px">
